...
class MunicipalityController extends \BaseController {
	...
	{
		if (Sentry::hasAnyAccess(['municipalities_create'])) return View::make('municipalities.admin.create');
		else return Redirect::route('pages.error');
	}
	...
	public function getAdminUpdate($id)
	{
		if (Sentry::hasAnyAccess(['municipalities_update'])) 
		{
			...
			if (Request::ajax()) return Response::json(['municipality' => $municipality]);
			else return View::make('municipalities.admin.edit', ['municipality' => $municipality]);
		} 
		else return Redirect::route('pages.error');
	}
